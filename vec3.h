...
class vec3 // a vector3 contains 3 floats
{
public:

	float f[3]; // just an array of 3 floats underneath

	vec3() :f{ 0,0,0 } {} // initializes the array to 0

	vec3(float f1, float f2, float f3) :f{ f1, f2, f3 } {}

	float x() const { return f[0]; } // ? - Why is const added in the function signature
	float y() const { return f[1]; }
	float z() const { return f[2]; }

	vec3 operator-() const { return vec3(-f[0], -f[1], -f[2]); } // Negate the vector
	float operator[](int i) const { return f[i]; } // one element
	float& operator[](int i) { return f[i]; } // one element
